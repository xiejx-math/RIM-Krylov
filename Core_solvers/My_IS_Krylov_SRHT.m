function [x,Out]=My_IS_Krylov_SRHT(A,b,q,opts)


% The iterative-sketching-based Krylov subspace method with subsampled randomized Hadamard transform (IS-Krylov-SRHT) for solving linear systems
%              Ax=b
...
X_SRHT = [A, b];
%% executing the IS-Krylov-SRHT method
stopc=0;
iter=-1;
times(1)=toc;
current_idx = 1;
...
while ~stopc
    ...
    while ~stopsampling
        ...
        %AindexR=A(indexR,:);
        %bindexR=b(indexR);
        % [AindexR,bindexR]=My_SRHT_sketch_C(A, b, q);
        [AindexR,bindexR]=My_SRHT_sketch_C(X_SRHT, m, n, q, sqrt(m/q));
