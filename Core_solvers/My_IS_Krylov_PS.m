function [x,Out]=My_IS_Krylov_PS(A,b,q,opts)

% The iterative-sketching-based Krylov subspace method with parition sampling (IS-Krylov-PS) for solving linear systems
%              Ax=b
...
if probset
    ...
    for i=1:tau
        ...
        blockArnormfro(i) = sum(diags_perm(ps));
        start_idx = start_idx + q;
    ...
    normArfro = sum(blockArnormfro);

    if normArfro > 0
        prob = blockArnormfro / normArfro;
    else
        prob = ones(tau,1) / tau;
    end
...
%% executing the IS-Krylov-PS method
stopc=0;
iter=-1;
times(1)=toc;
current_idx = 1;
while ~stopc
   ...
   elseif l==2
       ...
       elseif iter<=l-2&&iter>=1
           ...
           eta=eta./norm_Pk';
           pk=dk-Pk*eta;
           % if abs(norm(Pk'*Pk./norm_Pk)-1)>10^(-2)
           %     disp('nonorthogonal')
           % else
           %     fprintf('%d',iter)
           % end
